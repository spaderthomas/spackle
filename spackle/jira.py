...
def format_ticket_markdown(ticket: JiraTicket) -> str:
    ...
    # Title
    if ticket.key:
        display_title = ticket.summary if ticket.summary else ticket.title.replace(f'[{ticket.key}]', '').strip()
    ...
    # Project info
    if ticket.project:
        formatter.add_line(f"**Project:** {ticket.project.name} ({ticket.project.key}, ID: {ticket.project.id})")
        formatter.add_line_break()
...
def fetch_jira_xml_from_url(url: str, timeout: int = 10) -> str:
    jira_fields = [
        'key', 'summary', 'description', 'comments', 'customfield',
        'project', 'type', 'priority', 'status', 'resolution',
        'assignee', 'reporter', 'created', 'updated', 'resolved',
        'component', 'labels', 'environment', 'timespent', 'votes', 'watches'
    ]
