...
def run_probe_command(command: List[str]) -> str:
  ...
  try:
    result = subprocess.run(
      command, cwd=get_project_root(), capture_output=True, text=True, timeout=30
    )
...
def probe_server():
  ...
  @mcp.tool()
  def search_code(query: str, subpath: str = '', session: str = 'new') -> str:
  ...
  @mcp.tool()
  def query_code(pattern: str, subpath: str = None, language: str = None) -> str:
  ...
  @mcp.tool()
  def extract_code(files: List[str]) -> str:
    """
