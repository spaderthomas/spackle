...
class ServerPaths:
  def __init__(self):
    self.file: str = os.path.dirname(os.path.abspath(__file__))
    self.project: str = os.path.realpath(os.path.join(self.file, '..', '..', '..'))
    self.asset: str = os.path.join(self.project, 'asset')
    self.database: str = os.path.join(self.asset, 'spry.db')
...
def read_query(
    ...
    with SQLiteConnection(server_paths.database) as conn:
        cursor = conn.cursor()
