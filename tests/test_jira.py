...
class TestJiraDataclasses:
  ...
  def test_jira_comment_creation(self):
    comment = JiraComment(author='john.doe', created='2025-01-01', text='Test comment')
...
class TestMarkdownGeneration:
  ...
  def test_format_ticket_markdown_with_comments(self):
    ...
    comment = JiraComment(
      author='john.doe', created='2025-01-01', text='Test comment text'
    )
    ticket = JiraTicket(
      key='TEST-123',
      title='Test Issue',
...
class TestURLConversion:
  ...
  def test_convert_jira_browse_url_to_xml_basic(self):
    ...
    expected = (
      'https://example.atlassian.net/si/jira.issueviews:issue-xml/SLG-999/SLG-999.xml'
    )
